# Cronograma de Actividades (CRON)

Este directorio contiene los documentos que forman parte del Elemento de Configuración del Software (ECS) relacionado con el cronograma de actividades del proyecto **Healthy+**.

## Información del ECS

- **Código del ECS:** CRON
- **Nombre del ECS:** Cronograma de Actividades
- **Autor:** Marcelo Acuña, Abner Arboleda, Christian Bonifaz
- **Proyecto:** Healthy+
- **Línea base:** PDP - Plan de Desarrollo del Proyecto
- **Tipo de ECS:** Documento (XLSX)
- **Fecha de creación:** 08/01/2026
- **ID del proyecto:** 27837_G1_ADS

## Líneas Base Relacionadas

- **LBA - Análisis:** Contiene diagramas de clases
- **LBC - Implementación:** Contiene código fuente frontend y backend del proyecto
- **LBD - Diseño:** Contiene diagramas de casos de uso
- **LBR - Requisitos:** Contiene la especificación completa de requisitos

## Contenido Principal

- Plan de desarrollo del proyecto
- Cronograma de iteraciones
- Hitos y entregables
- Estimaciones de tiempo

## Fecha de inicio:

08 de enero de 2026

## Responsables:

Marcelo Acuña, Abner Arboleda, Christian Bonifaz

# Historial de Versiones del Cronograma

| Versión            | Fecha      | Responsable       | Aprobado por      |
| ------------------ | ---------- | ----------------- | ----------------- |
| `CRON_V3.0.0.xlsx` | 01/02/2026 | Christian Bonifaz | Christian Bonifaz |

Esta versión refleja el estado actual del cronograma del proyecto conforme al avance y cambios en el plan de desarrollo. Ha sido validada y aprobada como parte de la gestión del proyecto.