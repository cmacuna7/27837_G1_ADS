...
class AplicacionHealthy {
  ...
  configurarAlertas() {
    ...
    // Configurar filtros por paciente
    this.configurarFiltrosPaciente();
  }

  /**
   * Configura los filtros por paciente en todas las pantallas
   */
  configurarFiltrosPaciente() {
    // Filtro en lista principal
    const filtroListaPaciente = document.getElementById('filter-paciente-list');
    if (filtroListaPaciente) {
      filtroListaPaciente.addEventListener('change', async (e) => {
        await this.filtrarMedicamentosPorPaciente(e.target.value);
      });
    }

    // Filtro en historial
    const filtroHistoryPaciente = document.getElementById(
      'filter-paciente-history',
    );
    if (filtroHistoryPaciente) {
      filtroHistoryPaciente.addEventListener('change', (e) => {
        if (this.controladorHistorial) {
          this.controladorHistorial.filtros.paciente = e.target.value;
          this.controladorHistorial.renderizar();
        }
      });
    }

    // Filtro en síntomas
    const filtroSymptomsPaciente = document.getElementById(
      'filter-paciente-symptoms',
    );
    if (filtroSymptomsPaciente) {
      filtroSymptomsPaciente.addEventListener('change', (e) => {
        if (this.controladorSintomas) {
          this.controladorSintomas.filtrarPorPaciente(e.target.value);
        }
      });
    }
  }
  ...
  /**
   * Pobla los selectores de pacientes
   */
  async poblarSelectoresPaciente(medicamentos) {
    const pacientes = [
      ...new Set(
        medicamentos
          .map((m) => m.nombrePaciente)
          .filter((p) => p && p.trim() !== ''),
      ),
    ];

    const selectores = [
      document.getElementById('filter-paciente-list'),
      document.getElementById('filter-paciente-history'),
      document.getElementById('filter-paciente-symptoms'),
    ];
  ...
  /**
   * Filtra medicamentos por paciente
   */
  async filtrarMedicamentosPorPaciente(paciente) {
    const resultado = await this.controladorMedicamentos.obtenerTodos();
    if (resultado.exito) {
      let medicamentosFiltrados = resultado.medicamentos;
      if (paciente && paciente.trim() !== '') {
        medicamentosFiltrados = medicamentosFiltrados.filter(
          (m) => m.nombrePaciente === paciente,
        );
      }
      this.vistaLista.renderizar(medicamentosFiltrados);
    }
  }
  ...
  async cargarMedicamentos() {
    try {
      ...
      if (resultado.exito) {
        ...
        // Poblar selectores de paciente
        await this.poblarSelectoresPaciente(resultado.medicamentos);
      } else {
  ...
  async eliminarMedicamento(id) {
    try {
      const resultado =
        await this.controladorMedicamentos.eliminarMedicamento(id);
  ...
  configurarSelectorDosis() {
    ...
    if (btnMenos && btnMas && doseValue) {
      btnMenos.addEventListener('click', (e) => {
        e.preventDefault();
        let valor = parseInt(doseValue.value) || 0;
        if (valor > 1) {
          valor -= 100;
          if (valor < 1) valor = 1;
          doseValue.value = valor;
        }
      });
      ...
      doseValue.addEventListener('input', (e) => {
  ...
  configurarSelectorFrecuencia() {
    ...
    buttons.forEach((button) => {
      button.addEventListener('click', (e) => {
        ...
        // Limpiar frecuencia personalizada cuando se selecciona un botón
        if (customFrequency) {
    ...
    // Cuando se escribe en frecuencia personalizada, deseleccionar botones
    if (customFrequency) {
      customFrequency.addEventListener('input', (e) => {
        let valor = parseInt(e.target.value);
        if (!isNaN(valor)) {
          buttons.forEach((btn) => btn.classList.remove('active'));
          // Validar rango
          if (valor < 1) e.target.value = 1;
          if (valor > 24) e.target.value = 24;
          e.target.value = Math.floor(valor);
        }
      });
    }
