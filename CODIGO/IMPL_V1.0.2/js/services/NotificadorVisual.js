...
class NotificadorVisual extends Observador {
  ...
  mostrarAlertaVisual(medicamento, horario) {
    ...
    if (alertaSubtitle) {
      let textoAlerta = `${medicamento.nombre} ${medicamento.dosis}${medicamento.unidadDosis}`;
      if (
        medicamento.nombrePaciente &&
        medicamento.nombrePaciente.trim() !== ''
      ) {
        textoAlerta = `${medicamento.nombrePaciente} - ${textoAlerta}`;
      }
      alertaSubtitle.textContent = textoAlerta;
    }

    // Mostrar notas si existen
    const alertaNotes = document.getElementById('alert-notes');
    if (alertaNotes) {
      if (medicamento.notas && medicamento.notas.trim() !== '') {
        alertaNotes.textContent = medicamento.notas;
        alertaNotes.style.display = 'block';
      } else {
        alertaNotes.style.display = 'none';
      }
    }
