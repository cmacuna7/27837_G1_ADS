  ...
  async generarReportePDF(opciones = {}) {
    try {
      ...
      for (const [medicamentoId, eventosDelMed] of Object.entries(
        ...
        const nombrePaciente = medicamento?.nombrePaciente || '';

        // Nombre del medicamento y paciente
        doc.setFontSize(12);
        doc.setFont(undefined, 'bold');
        if (nombrePaciente && nombrePaciente.trim() !== '') {
          doc.text(`${nombreMed} (Paciente: ${nombrePaciente})`, margen, yPos);
        } else {
        ...
        // Listar eventos
        for (const evento of eventosDelMed) {
          ...
          let estado = evento.tipo === 'tomado' ? 'tomado' : 'omitido';
          if (evento.motivo && evento.motivo.trim() !== '') {
            estado += ` (${evento.motivo})`;
          }

          let texto = `  ${fecha} ${hora} - ${estado}`;
    ...
    if (opciones.paciente) {
      ...
      eventos = eventos.filter((evento) => {
        const medicamento = medicamentos.find(
          (m) => m.id == evento.medicamentoId,
        );
        return medicamento && medicamento.nombrePaciente === opciones.paciente;
      });
    }
